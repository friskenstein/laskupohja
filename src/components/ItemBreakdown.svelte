...
<section>
	<textarea
		class="mx-2 my-8 bg-yellow-200 print:hidden"
		cols="80"
		...
		<tbody>
			...
			{#each items as item (item)}
				<tr>
					...
					<td class="p-2 text-right">
						{fmtMoney(item[1] * item[3] * (item[4] * 0.01 + 1), currency)}
					</td>
				</tr>
			{/each}
			...
			<tr>
				...
				<td colspan="3" class="font-bold">
					{lang === 'fi' ? 'Yhteensä (alv 0%)' : 'Total excl. VAT'}
				</td>
				<td class="pr-2 pt-2 text-right font-bold">
					{fmtMoney(
						items.reduce((acc, item) => acc + item[1] * item[3], 0),
						currency
					)}
				</td>
			</tr>
			<tr>
				...
				<td class="pr-2 pt-2 text-right font-bold">
					{fmtMoney(
						items.reduce((acc, item) => acc + item[1] * item[3] * item[4] * 0.01, 0),
						currency
					)}
				</td>
			</tr>
			<tr>
				...
				<td colspan="2" class="border-t border-neutral-700 text-lg font-bold">
					{lang === 'fi' ? 'Maksettava yhteensä' : 'Total amount due'}
				</td>
